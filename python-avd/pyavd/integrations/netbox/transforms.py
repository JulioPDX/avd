...
"""Field transformation functions for AVD to NetBox data conversion."""
...
def slugify(value: str) -> str:
    """Convert a string to a NetBox-compatible slug."""
    if not value:
        return ""
...
def invert_bool(value: bool | None) -> bool | None:
    """Invert a boolean value (AVD shutdown=True -> NetBox enabled=False)."""
    if value is None:
        return None
    return not value
...
def map_interface_mode(mode: str | None) -> str | None:
    """Map AVD interface mode to NetBox interface mode."""
    if not mode:
        return None
...
def map_vlan_status(state: str | None) -> str:
    """Map AVD VLAN state to NetBox VLAN status."""
    if not state:
        return "active"
...
def parse_speed(speed: str | None) -> int | None:
    """
    Parse AVD speed string to NetBox speed in kbps.
...
def map_interface_type(intf_name: str) -> str:
    """
    Map interface name to NetBox interface type.
