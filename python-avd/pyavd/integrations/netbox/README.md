...
├── async_sync.py        # Main async sync class (AsyncAVDNetBoxSync)
...
├── client.py            # Async HTTP client for NetBox REST API (AsyncNetBoxClient)
...
**AsyncNetBoxClient** - High-performance async HTTP client using httpx with support for v1 and v2 API tokens:

```python
from pyavd.integrations.netbox import AsyncNetBoxClient

async with AsyncNetBoxClient("https://netbox.example.com", "nbt_xxx.yyy") as client:
    devices = await client.get("/api/dcim/devices/", params={"site": "dc1"})
```

**AsyncAVDNetBoxSync** - Main synchronization class:
...
async def sync_to_netbox():
    async with AsyncNetBoxClient("https://netbox.example.com", "nbt_xxx.yyy") as client:
        sync = AsyncAVDNetBoxSync(client, site_name="DC1", create_prerequisites=True)
...
class AsyncAVDNetBoxSync:
    def __init__(
        ...
        max_concurrent: int = 10,             # Concurrent device processing
    ...
    async def sync_all(self, configs: dict, node_types: dict = None) -> SyncResult

    # Individual sync methods
    async def sync_device(self, config: dict, node_type: str = None) -> SyncResult
    async def sync_interfaces(self, config: dict) -> SyncResult
    async def sync_vlans(self, config: dict) -> SyncResult
    async def sync_vrfs(self, config: dict) -> SyncResult
    async def sync_cables(self, configs: dict) -> SyncResult
    async def sync_primary_ip(self, config: dict) -> SyncResult

    # Additional sync methods
    async def sync_prefix(self, prefix: str, vrf_name: str = None, description: str = "") -> SyncResult
    async def sync_prefixes_from_config(self, config: dict) -> SyncResult
    async def sync_asn(self, asn: int | str) -> SyncResult
    async def sync_asns_from_config(self, config: dict) -> SyncResult
    async def sync_port_channels(self, config: dict) -> SyncResult
    async def sync_interface_vlan_associations(self, config: dict) -> SyncResult

    # Reconciliation
    async def reconcile_objects(self, dry_run: bool = None) -> SyncResult

    # Purge - delete ALL objects with managed tag
    async def purge_all(self, dry_run: bool = None) -> SyncResult
```
...
asyncio.run(sync_with_reconcile())
```
