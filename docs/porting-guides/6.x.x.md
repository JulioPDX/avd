...
| Python 3.10+ required | High | Yes - Upgrade Python |
| Ansible 2.15+ required | High | Yes - Upgrade Ansible |
| **[Common Changes](#common-changes)** | | |
| [Schema validation enforced on all keys](#schema-validation-is-enforced-on-all-keys-in-ansible-role-eos_designs-and-eos_cli_config_gen) | High | Yes - Fix any schema violations |
| [`ansible.utils` collection dependency removed](#the-ansibleutils-ansible-collection-dependency-was-removed-from-the-aristaavd-ansible-collection) | Low | Remove from requirements if present |
| [Jinja2 extensions no longer needed in `ansible.cfg`](#jinja2-extensions-are-no-longer-needed-in-ansiblecfg) | Low | Clean up config (optional) |
| [`arista.avd.natural_sort` requires `sort_key`](#changes-to-ansible-filter-plugin-aristaavdnatural_sort) | Medium | Update custom templates using filter |
| **[Breaking Changes (eos_designs)](#changes-to-ansible-role-aristaavdeos_designs)** | | |
| [`management_eapi.enabled` now required for eAPI](#eapi-is-no-longer-enabled-by-default-in-eos_designs) | High | Yes - Set `management_eapi.enabled: true` |
| [Management IP required for eAPI with OOB management](#a-management-ip-or-ipv6-is-now-required-to-enable-eapi-when-default_mgmt_method-oob) | High | Yes - Set `mgmt_ip` or change management network |
| [Enhanced conflict detection](#enhanced-conflict-detection-in-eos_designs) | High | Yes - Resolve any duplicate key conflicts |
| [BGP AS `float` autoconversion removed](#removed-autoconversion-of-bgp_as-from-float-under-node-config) | High | Yes - Change to `asdot` or `asplain` |
| [Deprecated data models removed](#removal-of-deprecated-avd-design-eos_designs-data-models) | High | Yes - Migrate to new data models |
| [`avd_eos_designs_unset_facts` removed](#removal-of-avd_eos_designs_unset_facts) | Medium | Remove from playbooks if used |
| [`avd_eos_designs_debug` renamed](#renaming-of-avd_eos_designs_debug-to-eos_designs_keep_tmp_files) | Low | Rename to `eos_designs_keep_tmp_files` |
| [`design.type` key removed](#removal-of-the-designtype-key) | Low | Remove from inventory |
| [Custom keys require `_` prefix](#custom-keys-starting-without-_-in-structured-config-are-no-longer-supported) | Medium | Prefix custom keys with underscore |
| **[Configuration Changes (eos_designs)](#changes-to-ansible-role-aristaavdeos_designs)** | | |
| [CloudVision validation requirements](#changes-in-validation-requirements-for-cloudvision-clusters) | Medium | Review if using CV integration |
| [`cv_topology` and `use_cv_topology` behavior](#changing-behavior-of-cv_topology-and-use_cv_topology) | Medium | Review if using CV topology |
| [cEOS management interface changed to `Management0`](#default-management-interface-changed-for-ceos-platforms) | Medium | Review cEOS deployments |
| [Local user `structured_config` changes](#changes-in-local-user-structured-configuration-for-no_password-and-disabled) | Medium | Review local user configurations |
| [`uplink_interfaces` length validation](#length-of-uplink_interfaces-uplink_switches-and-uplink_switch_interfaces-has-to-be-same) | Medium | Ensure uplink lists match |
| [`structured_config` for `l3_edge`/`core_interfaces`](#removal-of-structured_config-and-introduction-of-interface-specific-structured-config-keys-for-l3_edge-and-core_interfaces) | Medium | Migrate to new key names |
| [`p2p_links` requires exactly 2 nodes](#core_interfacesp2p_links-or-l3_edgep2p_links-list-keys-now-accepts-a-minimum-and-maximum-of-two-entries) | Medium | Review P2P link definitions |
| [BGP AS notation control (`as_notation`)](#configure-bgp-as-notation-to-asdot-or-asplain-using-bgp_as_notation) | Medium | Set explicit notation if needed |
| [OSPF/VRF override behavior changes](#in-network-services-vrf-ospf-configuration-it-is-not-possible-anymore-to-override-data-from-the-underlay-ospf-process) | Medium | Review OSPF configurations |
| [PIM-SM multicast changes](#change-of-the-default-behavior-for-pim-sm-multicast-for-core_interfacesp2p_links-and-l3_edgep2p_links) | Medium | Review multicast configurations |
| [WAN `wan_mode` key renamed](#the-wan_mode-autovpn-renamed-wan_mode-legacy-autovpn) | Medium | Update WAN configurations |
| [Jinja2 custom template removal](#removal-of-default-jinja2-templates-for-interface_descriptions-and-ip_addressing-in-eos_designs) | Medium | Migrate to Python custom templates |
| [`switch.*` variables removed from templates](#removing-support-for-templating-of-internal-switch-variables-in-jinja-templates) | Medium | Update custom templates |
| **Default Value Changes** | | |
| [BGP `maximum_routes` default: unlimited → 12000](#change-in-default-value-of-maximum-routes-for-underlay-bgp-peer-groups) | Medium | Review - may affect BGP sessions |
| `bgp_default_ipv4_unicast` default: false → true | Medium | Review BGP configurations |
| [SNMP Engine ID default changed](#the-default-value-for-snmp_settingscompute_local_engineid_source-is-now-rfc3411_type5) | Medium | Review SNMP configurations |
| [OSPF graceful restart default enabled](#the-default-value-for-underlay_ospf_graceful_restart-is-now-true) | Low | Review if explicit disable needed |
| **[eos_cli_config_gen Changes](#changes-to-the-ansible-role-aristaavdeos_cli_config_gen)** | | |
| [CLI configuration order changed](#cli-configuration-order-changes) | Medium | Review rendered configs |
| [New schema restrictions (strict validation)](#new-eos-config-eos_cli_config_gen-schema-restrictions) | High | Fix schema violations |
| `dhcp_servers.subnets` → `ipv4_subnets`/`ipv6_subnets` | Medium | Update DHCP configurations |
| [`errdisable.recovery.causes` is now a list](#errdisablerecoverycauses-is-now-a-list-of-dictionaries) | Medium | Update errdisable configurations |
| [`ntp.servers` VRF handling changed](#ntpserversvrf-moved-to-ntpvrf-since-all-ntp-servers-must-use-the-same-vrf) | Medium | Review NTP configurations |
| [`vlan_interfaces` IPv6 OSPF key renamed](#updated-the-ipv6-ospf-data-model-for-vlan_interfaces-to-support-process-id) | Medium | Update VLAN interface configs |
| [`dot1x_settings` new data model](#new-8021x-configuration-model) | Medium | Migrate dot1x configurations |
| **[anta_runner Changes](#changes-to-ansible-role-aristaavdanta_runner)** | | |
| [`avd_catalogs_allow_bgp_vrfs` removed](#the-role-variable-avd_catalogs_allow_bgp_vrfs-has-been-removed) | Medium | Use `avd_catalogs_skip_tests` instead |
| [`anta_report_hide_statuses` renamed](#the-role-variable-anta_report_hide_statuses-has-been-renamed-to-anta_report_exclude_statuses-for-clarity) | Low | Rename to `anta_report_exclude_statuses` |
| [User catalogs disabled by default](#execution-of-user-defined-catalogs-stored-in-user_catalogs_dir-is-now-disabled-by-default) | Medium | Set `anta_catalogs_user_path` to enable |
| **[PyAVD Changes](#changes-to-pyavd)** | | |
| [`pyavd.validate_inputs` signature changed](#changes-to-pyavdvalidate_inputs-and-pyavdvalidate_structured_config) | Medium | Update Python code using PyAVD |
| [`pyavd.get_avd_facts` signature changed](#changes-to-pyavdget_avd_facts) | Medium | Update Python code using PyAVD |
| [`pyavd.get_device_structured_config` changed](#changes-to-pyavdget_device_structured_config) | Medium | Update Python code using PyAVD |
| [`pyavd.get_device_test_catalog` changed](#changes-to-pyavdget_device_test_catalog) | Medium | Update Python code using PyAVD |
...
The following data model keys have been removed from AVD Design (`eos_designs`) data model in AVD 6.0.0.
Review the new model and update your input variables accordingly.
...
| `underlay_multicast` | `underlay_multicast_pim_sm` or `<node_type_key>.defaults.underlay_multicast.pim_sm.enabled` or `<node_type_key>.node_groups[].underlay_multicast.pim_sm.enabled` or `<node_type_key>.node_groups[].nodes[].underlay_multicast.pim_sm.enabled` or `<node_type_key>.nodes[].underlay_multicast.pim_sm.enabled` |
...
Starting with AVD 6.0.0, the following data model keys have new restrictions in AVD Design (`eos_designs`).
Review your input variables and ensure they comply with the new restrictions listed below.
...
Solution 1, leverage an available eos_designs data model if applicable:
...
!!! note
    The example above leverages the built-in defaults, if you would like to override any settings, please refer to the "New key" listed in the [Removal of deprecated AVD Design (eos_designs) data models](#removal-of-deprecated-avd-design-eos_designs-data-models) section above.
...
To retain the previous behavior, it is necessary to either leverage available variables for the underlay or to use custom_structured_configuration.
...
As of AVD 6.0.0, the global `underlay_multicast` is replaced with node setting `underlay_multicast.pim_sm.enabled` or global `underlay_multicast_pim_sm`.
...
### Removed support for the `wan_use_evpn_node_settings_for_lan` key
...
Starting with AVD 6.0.0, the following EOS Config data model keys will have defined restrictions in `eos_cli_config_gen`.
...
Starting with AVD 6.0.0, `radius_server.hosts` is used to generate the configs for `default` vrf and a new key `radius_server.vrfs` is introduced to generate configs for non-default vrfs, where the VRF name is the primary key, under which multiple hosts can be configured.
